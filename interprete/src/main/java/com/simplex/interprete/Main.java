...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		if (visitor.symbolTableO.size() > 1) {
			...
			System.out.println("Zs: "+visitor.Z);
			System.out.println("Gs: "+visitor.G);
			System.out.println("********************************************************************************************************");
	...
	public static String createS(SimplexCustomVisitor visitor){
		...
		System.out.println("S: "+ result + "+" + visitor.S.get(visitor.S.size()-1));
		System.out.println();

		return result;
	}
	public static void createZ(SimplexCustomVisitor visitor){
		ArrayList<Float> finalV = new ArrayList<Float>();
		finalV = visitor.allVs.get(visitor.allVs.size()-1);
		for (int i = 0; i < visitor.allOCoefs.size()-1; i++) {
			Float rta = (float) 0;
			for (int j = 0; j < visitor.allOCoefs.get(i).size(); j++) {
				rta += finalV.get(j)*visitor.allOCoefs.get(i).get(j);
			}
			visitor.Z.add(rta);
		}
	}
	
	public static void createG(SimplexCustomVisitor visitor){
		for (int i = 0; i < visitor.Z.size(); i++) {
			visitor.G.add((visitor.Z.get(i)-visitor.minPaymentM.get(i))/visitor.differences.get(i));
		}
	}
}
