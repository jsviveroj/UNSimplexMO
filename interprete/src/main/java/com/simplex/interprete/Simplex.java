...
public class Simplex {
    public int rows, cols; // row and column
    public float[][] table; // simplex tableau
    private boolean solutionIsUnbounded = false;
	Map<Integer,Integer> inOutBaseVariables  = new HashMap<Integer,Integer>();
    ...
    public ERROR compute(int vTableSize){
        // step 1
        if(checkOptimality()){
        ...
        //System.out.println("Entra Columna: "+pivotColumn);
        // System.out.println("");
        ...
        //System.out.println("Sale Fila: "+ pivotRow);
        

        
        // Se verifica que no hayan variables dentro
        for (int i = 0; i < inOutBaseVariables.size(); i++) {
            if (pivotRow == inOutBaseVariables.get(i)) {
    			inOutBaseVariables.replace(i,-1);
    			break;
    		}
		}

        
        // Se añade el registro de las variables que entran
        if ( pivotColumn < vTableSize) {
        	inOutBaseVariables.put(pivotColumn,pivotRow);
		}
    ...
    private int findEnteringColumn(){
        ...
        for(pos = 0; pos < cols-1; pos++){
            if(table[rows-1][pos] < 0){
                ...
                count++;
                if (table[rows-1][pos] < temp) {
					temp = table[rows-1][pos];
					newLoc = pos;
				}
            }
        }
